...
GeneticAlgorithm.prototype = {
    ...
    createPopulation: function () {
        ...
        for (var i = 0; i < this.max_units; i++) {
            ...
            var newUnit = new synaptic.Architect.Perceptron(3, 9, 1);
    ...
    activateBrain: function (top, pedal, sayac) {
       
        var topX = this.normalize(top.x, 635) * this.SCALE_FACTOR;

       
        var topY = this.normalize(top.y, 463) * this.SCALE_FACTOR;

        //var topDX = this.normalize(top.dx,1) * this.SCALE_FACTOR;

        //var topDY = this.normalize(top.dy,1) * this.SCALE_FACTOR;

        var pedalY = this.normalize(pedal.y, 408) * this.SCALE_FACTOR;

        // create an array of all inputs
        var inputs = [topX, topY, pedalY];

      
        var outputs = this.Population[sayac].activate(inputs);

      
        if (outputs[0] > 0.5) {
            pedal.stopMovingDown();
            pedal.moveUp();
        }
        else {
            pedal.stopMovingUp();
